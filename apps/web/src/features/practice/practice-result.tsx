...
export function PracticeResult() {
  ...
  useEffect(() => {
    ...
    provider.getDiagnostics()
      .then((diagnostics) => {
        ...
        setData(result);
        setIntervention(result ? buildInterventionSummary(result.event, diagnostics.recentAttemptEvents) : null);
  ...
  return <><ResultSummary data={data} intervention={intervention} nextHref="/learn" onOpenReason={() => setReasonOpen(true)} /><ReasonDrawer onClose={() => setReasonOpen(false)} onCorrection={recordCorrection} open={reasonOpen} status={data.status} /></>;
}
