...
const pid = (s: string) => `11111111-1111-4111-8111-1111111111${s}`;
const iid = (s: string) => `22222222-2222-4222-8222-2222222222${s}`;
const aid = (s: string) => `33333333-3333-4333-8333-3333333333${s}`;
const cid = (s: string) => `44444444-4444-4444-8444-4444444444${s}`;

export const SEED_GOAL_ID = "66666666-6666-4666-8666-666666666601";
export const SEED_EXPLORATION_ID = "55555555-5555-4555-8555-555555555501";
...
function buildItems(): PracticeItem[] {
...
function buildAttempts(ownerUserId: string, items: PracticeItem[], now: Date): AttemptEvent[] {
  ...
  const push = (pointIdx: number, runs: SeedAttempt[]) => {
    ...
    for (const run of runs) {
      ...
      events.push({
        ...
        contentVersion: 1,
        schemaVersion: 1,
        createdAt: new Date(now.getTime() - 3 * DAY + n * 30 * 60000).toISOString(),
      });
    }
  };
  push(0, STABLE_RUN);
...
const CARD_CONTENT: Array<[string, string]> = [
...
function buildCards(ownerUserId: string, now: Date): { cards: ReviewCard[]; states: ReviewState[] } {
  ...
  CARD_CONTENT.forEach(([front, back], i) => {
    ...
    states.push({
      ...
      ease: 2.5,
      intervalDays: 0,
      dueAt: new Date(i < 3 ? now.getTime() - (i + 1) * HOUR : now.getTime() + (i - 2) * DAY).toISOString(),
      reps: 0,
      lapses: 0,
      lastGrade: null,
...
export function buildSeedBundle(ownerUserId: string, now: Date): SeedBundle {
  const nowISO = now.toISOString();
  const pastISO = new Date(now.getTime() - DAY).toISOString();
  const { cards, states } = buildCards(ownerUserId, now);
  return {
    goal: {
      id: SEED_GOAL_ID,
      ownerUserId,
      title: "高等数学（上）期末",
      scenario: "final",
      examDate: new Date(now.getTime() + 14 * DAY).toISOString().slice(0, 10),
      subjects: ["高等数学"],
      dailyMinutes: 45,
      courseId: null,
      archivedAt: null,
      strategyVersion: "goal-seed-1",
      createdAt: nowISO,
      updatedAt: nowISO,
    },
    practiceItems: buildItems(),
    attemptEvents: buildAttempts(ownerUserId, buildItems(), now),
    reviewCards: cards,
    reviewStates: states,
    exploration: {
      id: SEED_EXPLORATION_ID,
      ownerUserId,
      title: "熵到底是什么",
      status: "open",
      createdAt: pastISO,
    ...
    chatTurns: [
      {
        ...
        explorationId: SEED_EXPLORATION_ID,
        author: "user",
        aiRole: null,
        content: "熵到底是什么？能举个生活例子吗？",
        simulated: false,
        createdAt: pastISO,
      },
      {
        ...
        author: "ai",
        aiRole: "explainer",
        content: "想象一间不收拾的宿舍：东西只会越来越乱。熵就是“乱的程度”的度量。（模拟回复）",
        simulated: true,
        createdAt: pastISO,
      },
    ],
