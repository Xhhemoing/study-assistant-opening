...
#### 预览与实现提示词基线（2026-08-05 增补）

后续原型、设计生成和前端实现统一使用以下约束，避免“只有长文内容，缺少笔记工作台细节”的假象：

```text
为 AIstudy 构建一个桌面优先、Notion 熟悉度高但不是克隆的学习型笔记页面。
页面必须像真实文档工作台而不是营销页或静态文章：
- 左侧是可折叠的工作区导航，含搜索、收藏、私有/共享页面树、新建页面、垃圾箱和用户区；
- 顶部是轻量页面操作栏，包含面包屑、可编辑页面标题、保存状态、撤销/重做、分享、更多；
- 页面正文从图标/封面、标题、页面属性表、正文块开始；属性必须可见且可编辑，不把元数据做成另一个大卡片；
- 每个可编辑内容块悬停时在左侧显示拖拽手柄和加号；加号打开按「基础内容 / 媒体与数据 / 学习闭环」分组的 slash/插入菜单；
- 选择文本后显示紧凑浮动格式工具栏；块菜单、格式工具栏、右侧检查器只在有上下文时出现；
- 右侧检查器在桌面保持可关闭的窄栏，展示目录、反向链接、来源锚点、AI candidate；不得占用主要写作宽度；
- 允许正文舒适宽度、全宽媒体/表格和 2–3 栏模板，但移动端始终按稳定阅读顺序线性化；
- 学习功能要嵌入现有块体系：概念、来源摘录、例题、反例、回忆提示、练习和错因都要有可辨识但克制的块样式；
- AI 仅显示范围、引用的块/来源、候选差异和接受/拒绝操作；不得把生成结果伪装成已写入的正式内容；
- 使用真实笔记内容、可点击控件、悬停/焦点/空状态；不使用巨型 hero、渐变背景、装饰性卡片堆叠或假数据仪表盘。
视觉基调：温和中性色背景、细边界、8px 以下圆角、紧凑但不拥挤、信息层级清晰。用图标承担常用操作并提供 aria-label/title。
```

**通用模板吸收原则：**

| 开源参考 | 已验证交互模式 | AIstudy 采用方式 | 不照搬的部分 |
|---|---|---|---|
| AFFiNE / BlockSuite | 文档页属性、页面/全宽宽度偏好、块作为可扩展基本单元 | 页面图标/属性区、宽度选择、块级语义与视图分离 | 无限画布与复杂数据视图不进入首期编辑器 |
| AppFlowy | slash menu、浮动格式工具栏、文字/块类型白名单、桌面与移动差异 | 按任务分组的插入菜单、选择后格式栏、移动端底部/抽屉入口 | 不把所有格式能力平铺到默认工具栏 |
| Outline | 写作优先的 WYSIWYG、Markdown 快捷键、内联链接与嵌入在块菜单可发现 | 安静正文、块级插入、链接/来源可追溯、阅读与编辑投影 | 不以 Markdown 作为唯一事实源，也不牺牲 block/revision identity |
| BlockNote | hover 时的 `+` 与拖拽手柄、slash 触发、按 group 顺序渲染菜单项 | 采用相同可发现性；`基础内容 / 媒体与数据 / 学习闭环` 为稳定分组 | 不让编辑器依赖库的 UI schema 作为产品的导出契约 |

来源：BlockNote Side Menu 与 Suggestion Menus 官方文档（2026-08-05）；AFFiNE `PageDetailEditor`、AppFlowy `editor_page.dart`、Outline `rich-markdown-editor` 开源仓库（2026-08-05）。这些参考只定义可用的交互惯例，不替代本项目的 `Document/Block/Revision`、candidate-only 和学习证据边界。
