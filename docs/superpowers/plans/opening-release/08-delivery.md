...
**Release decision:** Q01–Q03 cover the five-function foundation (M4), not all expanded user requirements. The 2026-09-14 scope adds mandatory email/DingTalk access, media understanding, course knowledge and proactive tutoring/planning; [Q04](11-proactive-acceptance.md) gates M5. Transcription or platform access may be temporarily blocked, but storage/manual import cannot count as those capabilities passing. Any reduced full-release scope requires explicit user agreement. Signed-off evidence means actual observed checks, not a model's assurance of perfection.
