...
- [ ] Test two accepts racing, same-key changed payload, rejected draft, incomplete timetable, task done elsewhere, newly inserted class and protected sleep. Commit migration0020 only after migration ordering passes. Do not use the original working tree's uncommitted plan-state code without separate review.
