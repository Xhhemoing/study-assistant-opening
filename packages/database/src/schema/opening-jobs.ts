...
export const openingJobs = pgTable("opening_jobs", {
  id: uuid("id").primaryKey().defaultRandom(),
  workspaceId: uuid("workspace_id").notNull().references(() => workspaces.id, { onDelete: "cascade" }),
  ownerUserId: uuid("owner_user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  key: text("key").notNull(), kind: text("kind").notNull(), payload: jsonb("payload").notNull(),
  result: jsonb("result"), state: text("state").notNull().default("queued"),
...
export const openingOutbox = pgTable("opening_outbox", {
  ...
  topic: text("topic").notNull(), payload: jsonb("payload").notNull(),
