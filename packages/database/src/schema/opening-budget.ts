...
export const openingBudgetReservations = pgTable("opening_budget_reservations", {
  ...
  purpose: text("purpose").notNull(), amountCents: integer("amount_cents").notNull(),
  requestId: text("request_id").notNull().unique(), state: text("state").notNull().default("reserved"),
